...
function recurseIteration(node){
    ...
    //rob this node
    let profit1 = node.val + getGapchildMaxProfix(node,true) +getGapchildMaxProfix(node,false);
...
function getGapchildMaxProfix(node,leftSide){
    if(leftSide == true){
        if(node.left == null) return 0;
            ...
            return recurseIteration(node.left.left) + recurseIteration(node.left.right);
        }
    }
    else{
        if(node.right == null) return 0;
        else{
            return recurseIteration(node.right.left) + recurseIteration(node.right.right);
        }
    }
}